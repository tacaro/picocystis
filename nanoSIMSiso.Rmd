...
```{r}
p_no3_nh4_active_raincloud <- roi_tbl_active %>% filter(N_source == "no3", cell_type != "Other", active == "Active") %>%
  ggplot(aes(x = Ratio_15N_12Cx14N_12C, fill = light, color = light)) +
  stat_slab(alpha = 0.6, height = 0.5) +
  stat_slab(data = roi_tbl_active %>% filter(N_source == "NH4", cell_type != "Other", active == "Active", light == "dark"), color = "darkblue", alpha = 0.7, fill = "darkblue") +
geom_point(data = roi_tbl_active %>% filter(N_source == "NH4", cell_type != "Other", active == "Active", light == "dark"), color = "darkblue", y = -.5, size = 10, shape = 124, alpha = 0.3) +
  stat_pointinterval(data = roi_tbl_active %>% filter(N_source == "NH4", cell_type != "Other", active == "Active", light == "dark"), color = "black", alpha = 0.5) +
  geom_point(y = -.5, size = 10, shape = 124, alpha = 0.3) +
  stat_pointinterval(alpha = 0.5, color = "black") +
  facet_wrap(N_source~light, ncol = 1, strip.position = "right") +
